...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  const admin = createSupabaseAdminClient();
  const { data: maxRow } = await admin.from("plant_climates").select("sort_order").order("sort_order", { ascending: false }).limit(1).maybeSingle();
  const sort_order = ((maxRow as { sort_order: number } | null)?.sort_order ?? 0) + 1;

  const { error } = await admin.from("plant_climates").insert({ key, label_es, label_en, active: true, sort_order });
